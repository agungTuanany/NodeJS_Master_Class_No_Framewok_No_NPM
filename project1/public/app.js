			...
			if (statusCode === 200) {
				...
				if (allchecks.lenght > 0) {
					...
					allchecks.forEach ( (checkId) => {
						...
						app.client.request (undefined, "api/users", "GET", newQueryStringObject, undefined, (statusCode, responsePayload) => {
							if (statusCode === 200) {
								...
								console.log ("Checks Page Error: Error trying to load check ID", checkId)
							}
						})
					})
				...
				// If the request comes back as a something than 200, log the user out (on the assumption that the api is temporarily down or the users token is bad)
				app.logUserOut ()
			}
		})
